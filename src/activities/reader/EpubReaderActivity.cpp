...
void EpubReaderActivity::onEnter() {
  ...
  if (Storage.openFileForRead("ERS", epub->getCachePath() + "/progress.bin", f)) {
    ...
    // visibleTextOffset: present in both 10-byte (v1.5, no status) and 11-byte (new) files
    if (dataSize == 10 || dataSize == 11) {
      cachedVisibleTextOffset = static_cast<uint32_t>(data[6])        |
                                (static_cast<uint32_t>(data[7]) << 8)  |
                                (static_cast<uint32_t>(data[8]) << 16) |
                                (static_cast<uint32_t>(data[9]) << 24);
    }
    // Status: legacy 7-byte file (status at byte 6) or new 11-byte file (status at byte 10)
    if (dataSize == 7) {
      currentStatus = static_cast<BookStatus>(data[6]);
      lastSavedSpineIndex = currentSpineIndex;
      lastSavedPage = nextPageNumber;
      LOG_DBG("ERS", "Loaded status (legacy): %d", static_cast<uint8_t>(currentStatus));
    } else if (dataSize == 11) {
      currentStatus = static_cast<BookStatus>(data[10]);
      lastSavedSpineIndex = currentSpineIndex;
      lastSavedPage = nextPageNumber;
      LOG_DBG("ERS", "Loaded status: %d", static_cast<uint8_t>(currentStatus));
    } else {
...
void EpubReaderActivity::openReaderMenu() {
  ...
  startActivityForResult(
      std::make_unique<EpubReaderMenuActivity>(
          ...
          !cachedBookmarks.empty(), currentStatus),   // ← add currentStatus
      [this](const ActivityResult& result) {
        const auto& menu = std::get<MenuResult>(result.data);
        applyOrientation(menu.orientation);
        toggleAutoPageTurn(menu.pageTurnOption);
        if (currentStatus != menu.status) {
          const bool crossingFinished = (menu.status == BookStatus::FINISHED) !=
          ...
          if (menu.status == BookStatus::NEW_CHAPTER_AVAILABLE) {
            ...
            if (menu.status != BookStatus::READING &&
                menu.status != BookStatus::MARKED_FOR_LATER) {
...
void EpubReaderActivity::loop() {
  if (!epub) { finish(); return; }

  // AvesO3: skip one frame after subactivity exit to prevent double-firing
  if (skipNextButtonCheck) {
    skipNextButtonCheck = false;
    return;
  }

  // Idle glyph prewarm (v1.5.0 block — unchanged)
  ...
  // Lazily resume partial extension build (v1.5.0 block)
  if (section && !section->isBuilding() && section->isPartial() && !RenderLock::peek() && buildViewportWidth > 0 &&
    ...
    if (!section->startBuild(buildSpec)) {
      ...
      LOG_DBG("ERS", "Reader near partial watermark (%d/%d), resuming extension build",
              section->currentPage, section->pageCount);
    }
  }

  // Background build tick (v1.5.0 block)
...
if (showBookmarkMessage && (millis() - bookmarkMessageTime) >= ReaderUtils::BOOKMARK_MESSAGE_DURATION_MS) {
  ...
  // Dictionary message timer (v1.5.0 addition)
  if (showDictionaryMessage && (millis() - dictionaryMessageTime) >= ReaderUtils::BOOKMARK_MESSAGE_DURATION_MS) {
    showDictionaryMessage = false;
    requestUpdate();
  }

  // endOfBookOptions menu — NON-AO3 books only.
  // AO3 fics have their own EOB screen and input handling below.
  if (atEndOfBook && !epub->hasAo3Info() && endOfBookOptions.menuActive() &&
  ...
  // Confirm: open reader menu (short press or touch swipe down from top edge).
  // Guard: don't open menu at EOB — EOB input is handled above and in the AO3 block below.
    ...
    if (ignoreNextConfirmRelease) {
      ...
      startActivityForResult(
          std::make_unique<EpubReaderMenuActivity>(
              renderer, mappedInput, epub->getTitle(), currentPage, totalPages,
              bookProgressPercent, SETTINGS.orientation, !currentPageFootnotes.empty(),
              !cachedBookmarks.empty(), currentStatus),
          [this](const ActivityResult& result) {
            const auto& menu = std::get<MenuResult>(result.data);
            applyOrientation(menu.orientation);
            toggleAutoPageTurn(menu.pageTurnOption);
            if (currentStatus != menu.status) {
              ...
              // Dashboard store hooks for manual status changes
              if (menu.status == BookStatus::NEW_CHAPTER_AVAILABLE) {
                NEW_CHAPTERS_STORE.loadFromFile();
                NEW_CHAPTERS_STORE.addBook(epub->getPath(), epub->getTitle(), epub->getAuthor());
                NEW_CHAPTERS_STORE.clearEntries();
...
// Handle input when at end of book
  if (currentSpineIndex >= epub->getSpineItemsCount()) {
    if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
      if (epub->hasAo3Info() && !epub->isAo3Completed()) {
        ...
        startActivityForResult(std::make_unique<AO3SyncActivity>(renderer, mappedInput, epub->getAo3WorkId(), epub->getAo3UpdateDate(), bookPath),
            ...
            if (!res.isCancelled && std::holds_alternative<AO3Result>(res.data)) {
              ...
              if (ao3Res.downloaded) {
                epub->saveAo3Info(epub->getAo3WorkId(), ao3Res.scrapedDate, ao3Res.isCompleted);
  ...
  // First page turn: promote START → READING
  if ((prevTriggered || nextTriggered) && currentStatus == BookStatus::START) {
    currentStatus = BookStatus::READING;
  }
  ...
  if (!section && currentSpineIndex < epub->getSpineItemsCount()) {
    requestUpdate();
   return;
  }
...
void EpubReaderActivity::jumpToPercent(int percent) {
  ...
  if (currentStatus == BookStatus::START) {
    currentStatus = BookStatus::READING;
  }
...
void EpubReaderActivity::render(RenderLock&& lock) {
  ...
  if (currentSpineIndex == epub->getSpineItemsCount()) {
    ...
    if (epub->hasAo3Info()) {
      // AO3 fics: custom EOB screen with series check
      ...
      if (!epub->isAo3Completed()) {
        ...
        if (ao3HasSeries) {
          ...
          int seriesY = mainY + 40 + lh12 + lh10;
          for (const auto& line : renderer.wrappedText(UI_10_FONT_ID, seriesBuf,
                                                        renderer.getScreenWidth() - 40, 2)) {
      ...
      // Non-AO3 books: next-book suggestion menu from v1.5.0
...
bool EpubReaderActivity::saveProgress(int spineIndex, int currentPage, int pageCount) {
  // Reset manual override if user moved significantly
  if (spineIndex != lastSavedSpineIndex || currentPage != lastSavedPage) {
    statusManuallySet = false;
  }

  // Automatic status detection (unchanged from HEAD)
  ...
  // Resolve visibleTextOffset for the new 11-byte format (from v1.5.0)
  std::optional<uint32_t> offset;
  if (section && spineIndex == currentSpineIndex &&
  ...
  // Write 11-byte progress file:
  //   [0-1] spineIndex  [2-3] pageNumber  [4-5] pageCount
  //   [6-9] visibleTextOffset  [10] BookStatus
  HalFile f;
  if (Storage.openFileForWrite("ERS", epub->getCachePath() + "/progress.bin", f)) {
    ...
    const uint32_t o = offset.value_or(0);
    data[6]  = o & 0xFF;
    data[7]  = (o >> 8)  & 0xFF;
    data[8]  = (o >> 16) & 0xFF;
    data[9]  = (o >> 24) & 0xFF;
    data[10] = static_cast<uint8_t>(currentStatus);
    f.write(data, 11);
    f.close();
    LOG_DBG("ERS", "Progress saved: Chapter %d, Page %d, Status %d",
            spineIndex, currentPage, static_cast<uint8_t>(currentStatus));
    return true;
  } else {
...
// Keep v1.5.0's rememberCurrentContentOffset() exactly as-is after saveProgress()
